...
those %ID of the new assembled sequences to have aligned at and the 
minimum length ratio. This means if you give it a value of 80 for %ID 
threshold and ratio of 0.5 for length then only %ID alignments from 
assemblies less than 80 and length ratio less than 0.5 will be put into 
...
include a locus that has already been assembled well enough. 
    ...
    Input:
        ...
        6. Path to where the unbuffered FASTA from extract_sequences.py is 
        7. Optional minimum length ratio of an assembled sequence as cutoff 
...
def main():
    ...
    parser.add_argument('--original_fsa', '-ofsa', type=str, required=True, help='Path to where the unbuffered FASTA from extract_sequences.py is.')
    ...
    with open(args.ivc,'r') as i:
        for line in i:
            line = line.rstrip()
            result = line.split('\t') 
            length_assmb = int(result[2])
            filename = re.search(regex_for_locus,result[5]).group(1)
            locus = filename.split('.')[1]
            locus_key = filename.split('.WITH')[0]
            unbuffered_ref_seq = seq_dict[locus_key]
            ...
            if float(percent_id) >= args.threshold and length_assmb >= int(len(unbuffered_ref_seq)*min_len):
