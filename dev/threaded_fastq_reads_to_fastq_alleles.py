...
def main():
    ...
    # Use three threads to do this last part, one to iterate over the FASTQ 
    # files and the other two to write out the reads of each file.
    manager = mp.Manager()
    q = manager.Queue()
    pool = mp.Pool(2)
    ...
    job = pool.apply_async(filter_fastq, (ids_to_keep,filename,output,q))
    jobs.append(job)

    for job in jobs:
        job.get()

    q.put('stop') # done multiprocessing, clean up 
    pool.close()
    pool.join()
...
def listener(queue):
    ...
    # Listens for messages and writes the FASTQ entries
    while 1:
        # FASTQ entries will come in one line at a time (total entry == 4 lines)
        # with the final message being the directory to write to.
        msg = queue.get() 
        ...
        if msg == 'stop':
            ...
            # If we have 5 lines, we have all we need to write out the FASTQ entry
            if msg_len == 5:
                dir = msg_list.pop()

                with open(dir,'ab',0) as out:
                    for line in msg_list:
                        out.write(line)
                        out.flush()

                msg_len = 0 # reinitialize
                msg_list = []
...
# queue = the shared queue between this worker and the listener where
# the FASTQ data will be place. 
def filter_fastq(ids,fastq,outdir,queue):
                ...
                if lineno == 4: # got a FASTQ entry, check if it's relevant
                    ...
                    if id in ids: # if relevant, write to all necessary directories/files
                        ...
                        for ref in ids[id]:
                            ...
                            # Place the four lines of the relevant FASTQ entry
                            for line in entry1:
                                queue.put(line.encode()) # encode so that it fits when sending to pool
                            queue.put(out1) # send the reference/directoryname
                            
                            for line in entry2:
                                queue.put(line.encode())
                            queue.put(out2)
