...
def main():
    ...
    parser.add_argument('-needle_install', type=str, required=True, help='Path to install directory of EMBOSS needle executable (e.g. /path/to/packages/emboss/bin/needle).')
    ...
    with open(args.assmb_map,'r') as dir_map:
        ...
        for line in dir_map:
            ...
            if args.assmb_type == "SPAdes":
                ...
                jobs.append(pool.apply_async(worker, (locus,contigs,ref_dict[locus],seq_dict,out_dir,min_len,max_len,q,args.assmb_type,args.priority,args.needle_install)))
            else:
...
# needle_exe = path to EMBOSS needle executable
def worker(locus,contigs,ref_list,seq_dict,out_dir,min_len,max_len,queue,assmb_type,priority,needle_exe):
    ...
    for record in SeqIO.parse(contigs,"fasta"):
        ...
        for ref_seq in ref_list:
            ...
            alignment = align(out_dir,ref_seq,record.id,aseq_file,bseq_file,'f',assmb_type,needle_exe)
            ...
            if assmb_type == "SPAdes":
                ...
                # Now have the reference FASTA file, perform alignment
                # with the assembled contig.
                alignment = align(out_dir,ref_seq,record.id,aseq_file,bseq_file,'r',assmb_type,needle_exe)
...
# assmb_type = either "SPAdes" or "HGA"
# needle_exe = path to EMBOSS needle executable
def align(out,allele,contig,aseq,bseq,f_or_r,assmb_type,needle_exe):
